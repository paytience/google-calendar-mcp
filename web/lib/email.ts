  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
        ...
        <p style="color: #a1a1aa; font-size: 14px; margin-bottom: 8px;">Add this to your MCP config (Claude Code, Cursor, or Windsurf):</p>
        <div style="background: #18181b; border: 1px solid #27272a; border-radius: 8px; padding: 12px; margin-bottom: 24px;">
          <code style="font-size: 13px; color: #e4e4e7;">"OUTLOOK_MCP_API_KEY": "${apiKey}"</code>
        </div>
...
export async function sendSetupLinkEmail(to: string, setupUrl: string) {
  await getResend().emails.send({
    from: "Outlook MCP <noreply@mcpoutlook.com>",
    to,
    subject: "Complete your Outlook MCP setup",
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
