...
export function ChatPanel({ phase }: { phase: DemoPhase }) {
  ...
  return (
    <div className="w-full h-full flex flex-col rounded-xl bg-zinc-950 border border-zinc-800 overflow-hidden">
      ...
      {/* Chat content */}
      <div className="flex-1 p-5 font-mono text-sm text-left space-y-3 overflow-hidden">
        {/* User message */}
          ...
          >
            ...
            >
              <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-blue-500/10 border border-blue-500/20">
                <span className="text-[11px] text-blue-400 font-medium">create_calendar_event</span>
              </div>
              <pre className="mt-2 text-[11px] text-zinc-500 leading-relaxed">{TOOL_PARAMS}</pre>
            </motion.div>
          </motion.div>
        )}
